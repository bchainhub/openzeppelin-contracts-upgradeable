...
import "../../../src/token/CRC721/CRC721Upgradeable.sol";
import "../../../src/token/CRC721/ICRC721ReceiverUpgradeable.sol";
import "../../../src/utils/ChecksumUpgradeable.sol";

contract CRC721UpgradeableMock is CRC721Upgradeable {
...
contract CRC721ReceiverUpgradeableMock is ICRC721ReceiverUpgradeable {
...
contract NonCRC721ReceiverUpgradeableMock {}

contract CRC721UpgradeableTest is Test {
    event Received(address operator, address from, uint256 tokenId, bytes data);

    CRC721UpgradeableMock private _token;
    ...
    function setUp() public {
        ...
        _approved = makeAddr("approved");
        _operator = makeAddr("operator");
        _other = makeAddr("other");

        _token = new CRC721UpgradeableMock();
    ...
    function testTransferToChecksumZeroReverts() public {
        ...
        _token.transferFrom(_owner, ChecksumUpgradeable.zeroAddress(), _FIRST_TOKEN);
    }

    function testMintToChecksumZeroReverts() public {
        CRC721UpgradeableMock token = new CRC721UpgradeableMock();
        token.initialize("Non Fungible Token", "NFT");
        vm.expectRevert(bytes("CRC721: mint to the zero address"));
        token.mint(ChecksumUpgradeable.zeroAddress(), 1);
    }

    function testSafeTransferToReceiver() public {
        bytes4 magic = _selector("onERC721Received(address,address,uint256,bytes)");
        CRC721ReceiverUpgradeableMock receiver = new CRC721ReceiverUpgradeableMock(magic, false, false);
    ...
    function testSafeTransferToNonReceiverReverts() public {
        NonCRC721ReceiverUpgradeableMock nonReceiver = new NonCRC721ReceiverUpgradeableMock();
        vm.prank(_owner);
