...
import "../../../src/utils/cryptography/EIP712Upgradeable.sol";
import "../../../src/utils/cryptography/EDDSAUpgradeable.sol";
import "../../../src/proxy/ClonesUpgradeable.sol";

contract EIP712VerifierUpgradeableMock is EIP712Upgradeable {
    ...
    function verify(bytes memory signature, address signer, address mailTo, string memory mailContents) external view {
        ...
        address recoveredSigner = EDDSAUpgradeable.recover(digest, signature);
...
contract EIP712UpgradeableTest is Test {
    ...
    function testDomainSeparatorCloneShort() public {
        EIP712VerifierUpgradeableMock implementation = new EIP712VerifierUpgradeableMock();
        address clone = ClonesUpgradeable.clone(address(implementation));
        EIP712VerifierUpgradeableMock proxy = EIP712VerifierUpgradeableMock(clone);
        proxy.initialize(SHORT_NAME, SHORT_VERSION);
    ...
    function _deploy(string memory name, string memory version) private returns (EIP712VerifierUpgradeableMock) {
        EIP712VerifierUpgradeableMock eip712 = new EIP712VerifierUpgradeableMock();
